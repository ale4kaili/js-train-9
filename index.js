...
function findIndexByProperty(arr, name) {
  ...
  let i = arr.findIndex(element => element.name === name);
  return i;
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  let arrSmall = arr.slice(startIdx);
...
function removeAndDouble(arr, startIdx) {
  ...
  let newArray = arr.splice(startIdx);
  return arr.map(newArray => newArray * 2);
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function sumAndPrint(arr) {
  ...
  const initial = 0;
  const result = arr.reduce((accumulator, currentValue) => accumulator + currentValue, initial);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let map1 = flatten.map(element => element ** 2);
  let result = map1.filter(element => element > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = elements.reduceRight((accumulator, currentValue) => accumulator + currentValue, 0); // Обчислюємо суму останніх n елементів
...
function wordLengths(arr) {
  ...
  const words = arr.map(word => word.length);
  return words;
...
function findLongestWord(arr) {
  ...
  const longWord = arr.reduce((element, currentValue) => {
    if (currentValue.length > element.length) {
      return currentValue;
    } else {
      return element;
    }
...
function findDuplicateElements(arr) {
  ...
  const duplicate = arr.filter((value, index, self) => {
...
function calculateTotalPrice(arr) {
  ...
  const total = arr.reduce((sum, product) => {
